...
(def keyboard-z-offset 60)               ; controls overall height; original=9 with centercol=3; use 16 for centercol=2
...
(def model-right (difference
                   ...
                   (for [z (range 10 80 15)]
                     (for [y (range -15 90 15)]
                       (->> (cube 30 10 10)
                            (translate [(+ -50 (* 0.3 y)) y z]))))
                  ))
...
(def case-2
  (difference
   thumb-rest-wall
   (translate [0 0 50] (cube 300 300 60))))

(spit "things/case-2.scad"
      (write-scad
       case-2))
